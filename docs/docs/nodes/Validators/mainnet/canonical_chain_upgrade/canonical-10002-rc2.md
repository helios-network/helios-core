...
2. **Not-so-happy path**  
Validators have trouble upgrade chain. This could be some consensus breaking changes not covered in upgrade handler, or compatibility issue of the migrated state with new heliades binary, but validators can at least export the genesis.
...
1. Verify you are currently running the correct version (`eb018590`) of `heliades`:
   ```bash
   $ heliades version
   commit: eb018590
...
4. Verify you are currently running the correct version (`096cbe5`) of `heliades` after downloading the 10002-rc2 release:
    ```bash
   $ heliades version
   Version dev (096cbe5)
