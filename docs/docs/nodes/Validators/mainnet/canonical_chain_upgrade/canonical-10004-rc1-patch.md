...
3. Start your node with the new heliades binary to fulfill the upgrade.
...
2. Not-so-happy path  
Validators have trouble upgrading the chain. This could be some consensus-breaking changes not covered in the upgrade handler, or compatibility issue of the migrated state with new heliades binary, but validators can at least export the genesis.
...
It is critically important to back up the `.heliades/data/priv_validator_state.json` file after stopping your heliades process. This file is updated every block as your validator participates in consensus rounds. It is a critical file needed to prevent double-signing, in case the upgrade fails and the previous chain needs to be restarted.

## Upgrade Procedure

1. Verify you are currently running the correct (pre-upgrade) version (`94583db`) of `heliades`:
   ```bash
   $ heliades version
   Version dev (94583db)
...
4. Verify you are currently running the correct new version (`30b3dbf`) of `heliades` after downloading the 10004-rc1 release:
    ```bash
   $ heliades version
   Version dev (30b3dbf)
