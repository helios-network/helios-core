...
func (s *PrecompileTestSuite) TestUnbondingDelegation() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			s.Require().NoError(err)
			_, _, err = s.network.App.StakingKeeper.Undelegate(s.network.GetContext(), s.keyring.GetAddr(0).Bytes(), valAddr, math.LegacyNewDec(1), s.network.GetDenom(), math.NewInt(0))
			s.Require().NoError(err)
