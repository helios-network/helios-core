...
func (h Hooks) AfterValidatorBonded(ctx context.Context, _ sdk.ConsAddress, validator sdk.ValAddress) error {
	...
	for _, counterpartyChainParams := range params.CounterpartyChainParams {
		...
		hyperionId := counterpartyChainParams.HyperionId

		ev := h.k.GetLastEventByValidatorByHyperionID(sdkCtx, validator, hyperionId)
		...
		lowestObservedNonce := h.k.GetLastObservedEventNonceForHyperionID(sdkCtx, hyperionId)
		...
		h.k.setLastEventByValidatorByHyperionID(
			sdkCtx,
			validator,
			lowestObservedNonce,
			h.k.GetLastObservedEthereumBlockHeightForHyperionID(sdkCtx, hyperionId).EthereumBlockHeight,
			hyperionId,
		)
	}
