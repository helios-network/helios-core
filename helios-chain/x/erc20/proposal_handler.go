...
func NewErc20ProposalHandler(k keeper.Keeper) govtypes.Handler {
	return func(ctx sdk.Context, content govtypes.Content) error {
		...
		default:
			return errorsmod.Wrapf(sdkerrors.ErrUnknownRequest, "unrecognized erc20 proposal content type: %T", c)
		}
	}
}
func HandleAddNewAssetConsensusProposal(ctx sdk.Context, k keeper.Keeper, p *types.AddNewAssetConsensusProposal) error {
	...
	for _, asset := range p.Assets {
		...
		if err != nil {
			return errorsmod.Wrapf(types.ErrAssetNotFound, "failed to check if ERC20 contract exists for asset %s: %v", asset.Denom, err)
		}
		if !exist {
			return errorsmod.Wrapf(types.ErrAssetNotFound, "failed to add asset %s in consensus whitelist as the ERC20 contract does not exist", asset.Denom)
		}
		...
		if err := k.AddAssetToConsensusWhitelist(ctx, *asset); err != nil {
			return errorsmod.Wrapf(err, "failed to add asset %s to consensus whitelist", asset.Denom)
		}
	}
...
func HandleRemoveAssetConsensusProposal(ctx sdk.Context, k keeper.Keeper, proposal *types.RemoveAssetConsensusProposal) error {
	...
	for _, denom := range proposal.Denoms {
		if !k.IsAssetWhitelisted(ctx, denom) {
			return errorsmod.Wrapf(types.ErrAssetNotFound, "asset %s is not whitelisted", denom)
		}
...
func HandleUpdateAssetConsensusProposal(ctx sdk.Context, k keeper.Keeper, proposal *types.UpdateAssetConsensusProposal) error {
	...
	for _, update := range proposal.Updates {
		...
		if !k.IsAssetWhitelisted(ctx, update.Denom) {
			return errorsmod.Wrapf(types.ErrAssetNotFound, "asset %s is not whitelisted", update.Denom)
		}

		asset, err := k.GetAssetFromWhitelist(ctx, update.Denom)
		if err != nil {
			return errorsmod.Wrapf(err, "failed to retrieve asset %s from whitelist", update.Denom)
		}
		...
		if err := k.UpdateAssetInConsensusWhitelist(ctx, updatedAsset); err != nil {
			return errorsmod.Wrapf(err, "failed to update asset %s in whitelist", update.Denom)
		}
		...
		if err := k.UpdateAssetNativeSharesWeight(ctx, update.Denom, percentFactor, increaseWeight, 0); err != nil {
			return errorsmod.Wrapf(err, "failed to update native delegation shares weight: %s", err)
		}
	}
