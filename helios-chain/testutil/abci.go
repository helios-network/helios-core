...
//  3. EndBlock
//  4. Commit
func Commit(ctx sdk.Context, app *app.HeliosApp, t time.Duration, vs *cmttypes.ValidatorSet) (sdk.Context, error) {
...
func CommitAndCreateNewCtx(ctx sdk.Context, app *app.HeliosApp, t time.Duration, vs *cmttypes.ValidatorSet) (sdk.Context, error) {
	header, err := commit(ctx, app, t, vs)
	if err != nil {
		return ctx, err
	}
...
func DeliverTx(
	...
	return BroadcastTxBytes(appHelios, txConfig.TxEncoder(), tx)
}
...
func DeliverEthTx(
	...
	res, err := BroadcastTxBytes(appHelios, txConfig.TxEncoder(), tx)
	if err != nil {
		return res, err
	}
...
func CheckTx(
	...
	return checkTxBytes(appHelios, txConfig.TxEncoder(), tx)
}
...
func BroadcastTxBytes(app *app.HeliosApp, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ExecTxResult, error) {
...
func commit(ctx sdk.Context, app *app.HeliosApp, t time.Duration, vs *cmttypes.ValidatorSet) (tmproto.Header, error) {
...
func checkTxBytes(app *app.HeliosApp, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseCheckTx, error) {
	bz, err := txEncoder(tx)
	if err != nil {
		return abci.ResponseCheckTx{}, err
	}
